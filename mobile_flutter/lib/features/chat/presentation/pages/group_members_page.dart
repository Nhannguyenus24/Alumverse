...
class GroupMembersPage extends ConsumerStatefulWidget {
  const GroupMembersPage({super.key, required this.groupId, this.groupTitle});
...
class _GroupMembersPageState extends ConsumerState<GroupMembersPage> {
  ...
  Future<void> _showRenameDialog() async {
    ...
    try {
      await ref
          ...
          .updateGroup(widget.groupId, trimmed);
      ref.invalidate(chatListProvider);
      if (!mounted) return;
  ...
  Future<void> _showAddMembersSheet(
    ...
    final added = await showModalBottomSheet<List<int>>(
      ...
      builder:
          (_) => _AddMembersSheet(alreadyIn: currentIds, maxSelect: maxAdd),
    );
    ...
    try {
      ...
      if (context.mounted) {
        AppToast.success(
          context,
          'chat.added_members'.tr(
            namedArgs: {'count': added.length.toString()},
          ),
        );
      }
    } catch (e) {
...
class _MemberTile extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final resolved =
        member.avatarUrl != null ? resolveImageUrl(member.avatarUrl!) : null;
...
class _AddMembersSheet extends ConsumerStatefulWidget {
  const _AddMembersSheet({required this.alreadyIn, required this.maxSelect});
  ...
  void _toggle(Connection conn) {
    if (_selected.contains(conn.peerMemberId)) {
      ...
      if (_selected.length >= widget.maxSelect) {
        AppToast.info(
          context,
          'chat.max_add_toast'.tr(
            namedArgs: {'max': widget.maxSelect.toString()},
          ),
        );
        return;
      }
