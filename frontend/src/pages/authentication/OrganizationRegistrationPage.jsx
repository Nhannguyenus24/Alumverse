...
const getValidationSchema = (isAllOptional, isAcademicOptional) => z.object({
  ...
  studentCode: isAllOptional 
    ? z.string().optional().or(z.literal("")) 
    : z.string().min(1, "Mã số sinh viên là bắt buộc"),
  className: isAcademicOptional 
    ? z.string().optional().or(z.literal("")) 
    : z.string().min(1, "Hệ đào tạo là bắt buộc"),
  ...
  degreeType: isAcademicOptional
    ? z.string().optional().or(z.literal(""))
    : z.string().min(1, "Chuyên ngành là bắt buộc"),
});
...
const OrganizationRegistrationPage = () => {
  ...
  const schema = useMemo(
    () => getValidationSchema(isAllOptional, isAcademicOptional),
    [isAllOptional, isAcademicOptional]
  );
  ...
  const organizationId =
    organization?.id ??
    (Number.isInteger(parsedQueryOrgId) ? parsedQueryOrgId : null);
  ...
  useEffect(() => {
    if (organizationId) {
      ...
      setError(
        "Organization ID is required. Please provide a valid organization.",
      );
    }
  ...
  useEffect(() => {
    ...
    const loadTrustedVerifiers = async () => {
      ...
      try {
        ...
        const verifiers = Array.isArray(response?.data?.data)
          ? response.data.data
          : [];
        if (!cancelled) {
  ...
  const onSubmit = async (data, verifierIdOverride) => {
    ...
    const verifierId =
      typeof verifierIdOverride === "string" ||
      typeof verifierIdOverride === "number"
        ? verifierIdOverride
      ...
      if (response?.data) {
        ...
        if (verifierId) {
          try {
            ...
            enqueueSnackbar("Yêu cầu xác thực đồng nghiệp đã được gửi.", {
              variant: "success",
            });
          } catch (peerError) {
            console.error("Failed to request peer verification", peerError);
            enqueueSnackbar(
              "Gửi yêu cầu xác thực thất bại, nhưng đăng ký tổ chức đã thành công.",
              { variant: "warning" },
            );
          }
        }
        ...
        if (proofFile) {
          try {
            ...
            enqueueSnackbar("Yêu cầu xác thực minh chứng đã được gửi.", {
              variant: "success",
            });
          } catch (proofError) {
            console.error("Failed to submit verification request", proofError);
            enqueueSnackbar("Gửi yêu cầu xác thực minh chứng thất bại.", {
              variant: "error",
            });
          }
        }
        ...
        >
          ...
          >
            ...
            <Typography variant="body1" color="textSecondary" sx={{ mb: 2 }}>
              Vui lòng cung cấp thông tin học thuật của bạn để đăng ký tham gia
              tổ chức.
            </Typography>
            ...
            {isAllOptional && (
              <Alert severity="info" sx={{ mb: 2 }}>
                Bạn đã chọn người xác thực, không cần điền các thông tin bên dưới.
              </Alert>
            )}
            ...
            <Box sx={{ mb: 1 }}>
              <Typography
                variant="subtitle1"
                fontWeight={600}
                color={!isAllOptional ? "primary.main" : "textSecondary"}
                sx={{ mb: 1 }}
              >
            ...
            <Box sx={{ mb: 1, mt: 2 }}>
              <Typography
                variant="subtitle1"
                fontWeight={600}
                color={!isAcademicOptional ? "primary.main" : "textSecondary"}
                sx={{ mb: 1 }}
              >
            ...
            >
              ...
              >
                <Stack direction="row" spacing={1.5} alignItems="flex-start">
                  ...
                  >
                    <Iconify icon="eva:shield-fill" width={22} height={22} />
                  </Avatar>
                  <Box>
                    <Typography
                      variant="h6"
                    ...
                    <Typography
                      ...
                      sx={{ mt: 0.5 }}
                    >
                      Chọn một người bạn quen biết trong tổ chức để xác thực
                      danh tính, giúp yêu cầu của bạn được phê duyệt nhanh hơn.
                    </Typography>
                  </Box>
                </Stack>
              </Box>
                  ...
                  <Stack spacing={1.25}>
                    ...
                    <Box
                      ...
                      sx={{
                        ...
                        cursor: "pointer",
                        border: (theme) =>
                          `1.5px solid ${selectedVerifierUserId === "" ? theme.palette.primary.main : theme.palette.divider}`,
                        bgcolor:
                          selectedVerifierUserId === ""
                            ? "primary.lighter"
                            : "transparent",
                    ...
                    >
                      ...
                      <Box sx={{ minWidth: 0 }}>
                        <Typography variant="subtitle2" noWrap>
                          Không chọn người xác thực
                        </Typography>
                        <Typography variant="caption" color="textSecondary">
                          Quản trị viên sẽ trực tiếp phê duyệt yêu cầu của bạn.
                        </Typography>
                      </Box>
                      {selectedVerifierUserId === "" && (
                        <Iconify
                    ...
                    {trustedVerifiers.length > 0 ? (
                      trustedVerifiers.map((verifier) => {
                        const isSelected =
                          String(verifier.userId) === selectedVerifierUserId;
                        const subtitle = [verifier.program, verifier.major]
                          .filter(Boolean)
                          .join(" · ");
                        ...
                        return (
                          ...
                          >
                            <Avatar
                              ...
                              alt={verifier.fullName || verifier.userName}
                            ...
                            <Box sx={{ minWidth: 0 }}>
                              <Typography
                                ...
                                noWrap
                                color={
                                  isSelected ? "primary.darker" : "text.primary"
                                }
                              >
                                {verifier.fullName || verifier.userName}
                              </Typography>
                  ...
                  >
                    ...
                    <Box component="span" fontWeight={700}>
                      {trustedVerifiers.find(
                        (v) => String(v.userId) === selectedVerifierUserId,
                      )?.fullName || "người xác thực"}
                    </Box>
                    . Yêu cầu sẽ được gửi đi sau khi bạn đăng ký.
                  </Alert>
                )}
              </Box>
            </Paper>
