...
public interface AlumniPostR2dbcRepository extends R2dbcRepository<AlumniPost, Integer> {
    ...
    @Query("SELECT * FROM alumni_posts WHERE organization_id = :organizationId AND (LOWER(title) LIKE LOWER(CONCAT('%', :keyword, '%')) OR LOWER(content) LIKE LOWER(CONCAT('%', :keyword, '%'))) AND is_hidden = false ORDER BY published_at DESC LIMIT :limit OFFSET :offset")
    ...
    @Modifying
    @Query("UPDATE alumni_posts SET is_hidden = false, published_at = CURRENT_TIMESTAMP WHERE id = :id")
