...
\par\vspace{3cm}
{\fontsize{15}{21.5}\selectfont\bfseries CẢI TIẾN HỆ THỐNG KẾT NỐI SINH VIÊN\\[6pt]
...
Hướng thứ nhất là tái thiết kế kỹ thuật. Nhóm chuyển hệ thống từ kiến trúc Microservices với chín dịch vụ Spring Boot, Eureka, API Gateway và MySQL sang kiến trúc Modular Monolith kết hợp Layered trên nền Spring WebFlux, R2DBC và PostgreSQL. Việc chuyển sang Modular Monolith khắc phục các tính năng còn lỗi, giảm đáng kể độ phức tạp và chi phí vận hành cho một nền tảng phi lợi nhuận của trường đại học, còn phần tối ưu hóa hiệu năng đến từ quyết định riêng biệt là chuyển mô hình xử lý sang bất đồng bộ với Spring WebFlux và R2DBC.
...
Cần nhìn nhận thẳng rằng kiến trúc Microservices tự thân có tiềm năng hiệu năng và khả năng chịu tải cao hơn Monolith ở quy mô lớn, do mỗi dịch vụ có thể được nhân bản và mở rộng độc lập theo đúng điểm nghẽn tải của riêng nó, thay vì toàn bộ ứng dụng phải mở rộng đồng thời như trong Monolith. Đề tài không lựa chọn Microservices không phải vì phủ nhận lợi thế này, mà vì bài toán đặt ra là một nền tảng vận hành nội bộ cho một trường đại học, với quy mô tải chưa đến ngưỡng cần khai thác khả năng mở rộng độc lập theo từng dịch vụ. Trong khi đó, chi phí vận hành chín dịch vụ rời của phiên bản 2024, gồm cơ chế định danh dịch vụ Eureka, cổng vào API Gateway, chi phí giao tiếp qua mạng giữa các dịch vụ và độ phức tạp khi giám sát, gỡ lỗi trên một hệ phân tán, là chi phí thực tế phải gánh ngay trong khi lợi ích về khả năng chịu tải lớn lại chưa được tận dụng đến. Đây là một đánh đổi có chủ đích giữa chi phí vận hành và tiềm năng mở rộng, chứ không phải Monolith mặc định tối ưu hơn Microservices về hiệu năng. Phần tối ưu hóa hiệu năng thực sự đo được ở đề tài, trình bày tại mục \ref{subsec:reactive} ngay sau đây, đến từ một quyết định độc lập là chuyển mô hình xử lý từ đồng bộ sang bất đồng bộ, áp dụng được cho cả hai lựa chọn kiến trúc triển khai.
...
Cả hai cách ứng dụng trên được chọn để giải quyết những nỗi đau cụ thể của người dùng, được phân tích chi tiết ở các mục tiếp theo, chứ không phải được đưa vào chỉ để bổ sung yếu tố công nghệ mới cho đề tài. Trích xuất thẻ kỹ năng từ hồ sơ năng lực và mô tả chuyên môn nhằm giảm rào cản điền hồ sơ thủ công, phục vụ trực tiếp nhu cầu quản lý dữ liệu sinh viên và cựu sinh viên một cách có hệ thống của nhóm quản trị. Cùng thuộc nhóm hỗ trợ quản trị, mô hình ngôn ngữ còn được dùng để đối chiếu hồ sơ xin xác minh học vấn, so sánh thông tin cựu sinh viên tự khai với nội dung trích xuất từ tài liệu minh chứng và đưa ra khuyến nghị duyệt hoặc từ chối, giúp quản trị viên rút ngắn thời gian xử lý từng yêu cầu xác minh. Trợ lý ảo FitBot dựa trên RAG nhằm hạn chế hiện tượng bịa đặt thông tin khi trả lời các câu hỏi về quy chế, thủ tục và thông tin học vụ, phục vụ trực tiếp nhu cầu được giải đáp học thuật của nhóm sinh viên, đúng nhóm nhu cầu đạt điểm quan tâm cao trong khảo sát nội bộ được trình bày ở Bảng \ref{tab:survey}. Kiểm duyệt nội dung và tự động gắn cờ bài đăng vi phạm hỗ trợ giảm tải công việc thủ công cho quản trị viên diễn đàn khi kiểm soát nội dung do người dùng đăng tải. Gợi ý duyệt bài đăng bằng mô hình ngôn ngữ hỗ trợ quản trị khoa ra quyết định nhanh hơn khi khối lượng tin tức và bài đăng cần duyệt tăng theo quy mô đa đơn vị của hệ thống.

Hai trong số các tính năng nêu trên được minh họa cụ thể qua các hình dưới đây.

\begin{figure}[htbp]\centering
\includegraphics[width=0.86\textwidth]{ai_suggest}
\caption{Giao diện AI hỗ trợ đối chiếu hồ sơ xác minh học vấn}\label{fig:aiverify}
\end{figure}

\begin{figure}[htbp]\centering
\includegraphics[width=0.86\textwidth]{fitbot}
\caption{Giao diện trợ lý ảo FitBot}\label{fig:aichatbot}
\end{figure}
...
Khoảng trống kỹ năng hiện nay càng làm bộc lộ rõ nhu cầu được cố vấn. Theo một khảo sát của Đại học Khoa học Xã hội và Nhân văn, Đại học Quốc gia Hà Nội, 70\% sinh viên chưa có định hướng nghề nghiệp cụ thể sau khi tốt nghiệp, còn khảo sát của TopCV cho thấy gần 76\% doanh nghiệp ưu tiên tuyển ứng viên đã có từ một đến hai năm kinh nghiệm thực tế \cite{skillgap}.
...
Kế thừa quá trình khảo sát của báo cáo AlumVerse 2024, nhóm xem xét một số nền tảng kết nối cựu sinh viên tiêu biểu. Hệ thống BKA Alumni của Trường Đại học Bách khoa Thành phố Hồ Chí Minh có nhiều tính năng tương tác kiểu mạng xã hội (thông báo, nhắn tin, tạo nhóm, tương tác bài viết) và hỗ trợ lưu trữ sách, video, nhưng thiếu mục vinh danh cựu sinh viên tiêu biểu. Hệ thống RMIT Vietnam Alumni cho phép đăng ký sự kiện và có mục tư vấn, nhưng chỉ phát triển trên nền web và hạn chế tương tác giữa các cựu sinh viên. Hệ thống IoBM Alumni có tính năng tìm việc và tạo story tương tự Facebook, nhưng thiếu mục tin tức và vinh danh. Hệ thống Oxford Alumni cho phép đăng ký sự kiện và tham gia nhiều nhóm, nhưng cũng chỉ phát triển trên nền web và hạn chế trao đổi giữa các thành viên.

Bảng \ref{tab:altsystems} tổng hợp lại kết quả so sánh chức năng đã thực hiện ở báo cáo AlumVerse 2024, làm cơ sở đối chiếu với phạm vi chức năng mà AlumVerse hiện tại kế thừa và mở rộng.

\renewcommand{\arraystretch}{1.2}
\begin{longtable}{|>{\raggedright\arraybackslash}p{4.4cm}|>{\centering\arraybackslash}p{1.6cm}|>{\centering\arraybackslash}p{1.6cm}|>{\centering\arraybackslash}p{1.6cm}|>{\centering\arraybackslash}p{1.6cm}|}
\caption{So sánh chức năng giữa AlumVerse và các nền tảng cựu sinh viên tham khảo (theo khảo sát của báo cáo AlumVerse 2024)}\label{tab:altsystems}\\
\hline \textbf{Chức năng} & \textbf{BKA Alumni} & \textbf{RMIT VN Alumni} & \textbf{IoBM Alumni} & \textbf{Oxford Alumni} \\ \hline
\endfirsthead
\hline \textbf{Chức năng} & \textbf{BKA Alumni} & \textbf{RMIT VN Alumni} & \textbf{IoBM Alumni} & \textbf{Oxford Alumni} \\ \hline
\endhead
Hỗ trợ đa nền tảng (web và mobile) & & & & \\ \hline
Đăng ký tham gia hoạt động, sự kiện & & \checkmark & \checkmark & \checkmark \\ \hline
Vinh danh cựu sinh viên tiêu biểu & & \checkmark & & \\ \hline
Tư vấn và cố vấn & & \checkmark & & \\ \hline
Tạo nhóm & \checkmark & & & \checkmark \\ \hline
Đăng bài viết trong nhóm & \checkmark & & \checkmark & \\ \hline
Nhắn tin giữa cựu sinh viên & \checkmark & & \checkmark & \\ \hline
Tìm kiếm cựu sinh viên & \checkmark & \checkmark & \checkmark & \\ \hline
\end{longtable}
\renewcommand{\arraystretch}{1}

Điểm chung của bốn nền tảng tham khảo là mạnh về truyền thông một chiều và quản lý sự kiện, nhưng không hệ thống nào hỗ trợ đồng thời cả bốn nhóm chức năng đa nền tảng, cố vấn, vinh danh và tương tác hai chiều giữa cựu sinh viên với sinh viên như AlumVerse hướng tới.
...
\begin{figure}[htbp]\centering
\begin{subfigure}{0.46\textwidth}\centering\includegraphics[width=\linewidth]{ref-gayquy-ueh}\caption{Trang gây quỹ tham chiếu}\end{subfigure}\hfill
\begin{subfigure}{0.46\textwidth}\centering\includegraphics[width=\linewidth]{ref-diendan-voz}\caption{Diễn đàn tham chiếu}\end{subfigure}
\vspace{6pt}
\begin{subfigure}{0.46\textwidth}\centering\includegraphics[width=\linewidth]{ref-vieclam-zalo}\caption{Cổng việc làm tham chiếu}\end{subfigure}
...
Hình \ref{fig:ucactors} khái quát hóa quan hệ kế thừa giữa các tác nhân: Sinh viên và Cựu sinh viên đều kế thừa toàn bộ quyền của Khách vãng lai, còn Quản trị viên cấp trường kế thừa toàn bộ quyền của Quản trị viên cấp khoa và mở rộng thêm ở phạm vi toàn hệ thống, đúng như mô tả phạm vi dữ liệu ở Bảng \ref{tab:actors}.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc0-actors}
\caption{Quan hệ khái quát hóa giữa các tác nhân của hệ thống}
\label{fig:ucactors}
\end{figure}

Như minh họa tại Hình \ref{fig:ucguest}, tác nhân Khách vãng lai chỉ được phép xem nội dung công khai (tin tức, bài viết cựu sinh viên tiêu biểu, vinh danh thành tích, sự kiện, giới thiệu chương trình đào tạo) và thực hiện các thao tác tài khoản nền tảng, gồm chọn đơn vị để truy cập, đăng ký kèm kích hoạt bằng mã OTP, đăng nhập kèm xác minh chống bot, và khôi phục mật khẩu qua email.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc1-guest}
\caption{Sơ đồ use case của tác nhân Khách vãng lai}
\label{fig:ucguest}
\end{figure}

Hình \ref{fig:ucstudent} trình bày nhóm use case về hồ sơ và xác minh của Sinh viên, kế thừa từ Khách vãng lai: quản lý hồ sơ cá nhân và nghề nghiệp, xem hồ sơ công khai của thành viên khác, thiết lập tùy chọn thông báo, đổi email có xác thực hai đầu, nộp minh chứng xác minh tài khoản, gửi yêu cầu cập nhật thông tin học vấn, xem nhật ký đăng nhập và nhận thông báo đẩy qua trung tâm thông báo.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc2a-student}
\caption{Sơ đồ use case của tác nhân Sinh viên: hồ sơ, xác minh và thiết lập cá nhân}
\label{fig:ucstudent}
\end{figure}

Hình \ref{fig:ucstudent2} tiếp tục với nhóm use case kết nối và nội dung của Sinh viên: tìm kiếm thành viên trong mạng lưới, gửi và phản hồi yêu cầu kết nối, nhắn tin cá nhân và nhắn tin nhóm có đính kèm ảnh/video, chặn và bỏ chặn người dùng, tham gia và tìm kiếm chủ đề diễn đàn nghề nghiệp, tìm việc làm, truy cập kho học liệu, lưu nội dung để xem sau, và xem chuyên trang phát triển sự nghiệp.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc2b-student}
\caption{Sơ đồ use case của tác nhân Sinh viên: kết nối, nhắn tin, diễn đàn và nội dung}
\label{fig:ucstudent2}
\end{figure}

Hình \ref{fig:ucstudent3} thể hiện nhóm use case về sự kiện, cố vấn, quỹ và trợ lý ảo của Sinh viên: quan tâm và đăng ký tham dự sự kiện kèm nhận vé điện tử, đăng ký làm người được cố vấn và ghép cặp người cố vấn, đặt lịch buổi cố vấn có kiểm tra khung giờ còn trống, tham gia buổi cố vấn qua phòng họp trực tuyến, đánh giá và báo cáo sự cố sau buổi, đóng góp cho quỹ của đơn vị, trả lời khảo sát, và hỏi đáp với trợ lý ảo.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc2c-student}
\caption{Sơ đồ use case của tác nhân Sinh viên: sự kiện, cố vấn, quỹ và trợ lý ảo}
\label{fig:ucstudent3}
\end{figure}

Hình \ref{fig:ucalumni} bổ sung các use case dành riêng cho Cựu sinh viên, tác nhân kế thừa toàn bộ quyền của Sinh viên: đăng ký và khai báo hồ sơ người cố vấn với gợi ý thẻ kỹ năng bằng AI và tự động điền hồ sơ từ tệp CV, khai báo khung giờ rảnh, duyệt/dời lịch/hủy buổi cố vấn, gắn liên kết phòng họp trực tuyến, đăng tin tuyển dụng, chia sẻ học liệu, gửi bài viết cựu sinh viên tiêu biểu và thành tích để vinh danh (đều phải chờ kiểm duyệt), cùng xác minh chéo cho cựu sinh viên khác.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc3-alumni}
\caption{Sơ đồ use case bổ sung của tác nhân Cựu sinh viên}
\label{fig:ucalumni}
\end{figure}

Hình \ref{fig:ucfadmin} cho thấy nhóm use case kiểm duyệt nội dung và quản lý sự kiện của Quản trị viên cấp khoa: duyệt bài viết/tin tức/thành tích, quản lý chuyên mục diễn đàn, kiểm duyệt và điều hành chủ đề (có hỗ trợ kiểm duyệt tự động bằng AI), tạo và quản lý vòng đời sự kiện, điểm danh bằng mã QR, gửi thư mời, thống kê và xuất báo cáo tham dự, cùng xem bảng điều khiển thống kê của đơn vị.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc4a-facultyadmin}
\caption{Sơ đồ use case của tác nhân Quản trị viên cấp khoa: kiểm duyệt nội dung và quản lý sự kiện}
\label{fig:ucfadmin}
\end{figure}

Hình \ref{fig:ucfadmin2} trình bày nhóm use case còn lại của Quản trị viên cấp khoa, liên quan đến người dùng, cố vấn, quỹ và khảo sát: duyệt hồ sơ người cố vấn, theo dõi và xử lý báo cáo buổi cố vấn, xác minh danh tính người dùng, duyệt yêu cầu cập nhật học vấn, quản lý tài khoản người dùng trong đơn vị, tạo và vận hành chiến dịch gây quỹ kèm đối soát giao dịch đóng góp và xem bảng điều khiển quỹ, cùng tạo phát khảo sát và tổng hợp kết quả.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc4b-facultyadmin}
\caption{Sơ đồ use case của tác nhân Quản trị viên cấp khoa: người dùng, cố vấn, quỹ và khảo sát}
\label{fig:ucfadmin2}
\end{figure}

Hình \ref{fig:ucsadmin} thể hiện nhóm use case về vòng đời và cấu hình đơn vị của Quản trị viên cấp trường, tác nhân kế thừa toàn bộ quyền của Quản trị viên cấp khoa: khởi tạo trang khoa, duyệt yêu cầu đăng ký tổ chức mới, cấu hình đường dẫn và phát hành đơn vị, khai báo chương trình và ngành đào tạo, tùy biến thương hiệu và giao diện, bật tắt từng nhóm tính năng theo đơn vị, cấu hình quyền riêng tư, và cấp/thu hồi tài khoản quản trị đơn vị.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc5a-schooladmin}
\caption{Sơ đồ use case bổ sung của tác nhân Quản trị viên cấp trường: vòng đời và cấu hình đơn vị}
\label{fig:ucsadmin}
\end{figure}

Hình \ref{fig:ucsadmin2} khép lại nhóm use case của Quản trị viên cấp trường ở phạm vi hạ tầng dùng chung toàn hệ thống: quản lý mẫu thư điện tử, quản lý kho tri thức của trợ lý ảo, quản lý nhà cung cấp mô hình ngôn ngữ, giám sát vận hành toàn hệ thống, và tra cứu nhật ký kiểm toán.

\begin{figure}[H]\centering
\includegraphics[width=\textwidth,height=0.42\textheight,keepaspectratio]{uc5b-schooladmin}
...
\begin{figure}[htbp]\centering
\begin{subfigure}{\textwidth}\centering\includegraphics[width=\linewidth]{rwd-vinhdanh-desktop}\caption{Màn hình rộng: lưới nhiều cột, thanh điều hướng trải ngang}\end{subfigure}
\vspace{10pt}
\begin{subfigure}{0.545\textwidth}\centering\includegraphics[width=\linewidth]{rwd-vinhdanh-tablet}\caption{Màn hình trung bình: lưới thu về ít cột, điều hướng gom vào ngăn kéo}\end{subfigure}\hfill
\begin{subfigure}{0.235\textwidth}\centering\includegraphics[width=\linewidth]{rwd-vinhdanh-mobile}\caption{Màn hình hẹp: một cột}\end{subfigure}
\caption{Cùng một trang vinh danh thành tích hiển thị trên ba nhóm kích thước màn hình}
\label{fig:responsive}
\end{figure}

\begin{figure}[htbp]\centering
\begin{subfigure}{\textwidth}\centering\includegraphics[width=\linewidth]{web-trang-chu}\caption{Trang chủ của một đơn vị}\end{subfigure}
\vspace{10pt}
\begin{subfigure}{\textwidth}\centering\includegraphics[width=\linewidth]{web-co-van}\caption{Danh sách người cố vấn}\end{subfigure}
\caption{Màn hình trang chủ và danh sách người cố vấn trên ứng dụng web}
\label{fig:webscreens}
\end{figure}

\begin{figure}[htbp]\centering
\begin{subfigure}{\textwidth}\centering\includegraphics[width=\linewidth]{web-dien-dan}\caption{Diễn đàn nghề nghiệp}\end{subfigure}
\vspace{10pt}
\begin{subfigure}{\textwidth}\centering\includegraphics[width=\linewidth]{web-admin-dashboard}\caption{Bảng điều khiển quản trị đơn vị}\end{subfigure}
...
\begin{figure}[htbp]\centering
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-01-chon-to-chuc}\caption{Chọn đơn vị}\end{subfigure}\hfill
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-02-trang-chu}\caption{Trang chủ}\end{subfigure}\hfill
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-04-su-kien}\caption{Danh sách sự kiện}\end{subfigure}
\caption{Màn hình chọn đơn vị, trang chủ và danh sách sự kiện trên ứng dụng di động}
\label{fig:mobilescreens}
\end{figure}

\begin{figure}[htbp]\centering
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-05-ve-qr}\caption{Vé điện tử kèm mã QR}\end{subfigure}\hfill
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-06-chat}\caption{Hội thoại nhắn tin}\end{subfigure}\hfill
\begin{subfigure}{0.315\textwidth}\centering\includegraphics[width=\linewidth]{mob-08-quyen-gop}\caption{Danh sách quỹ quyên góp}\end{subfigure}
...
Trợ lý ảo được triển khai như một dịch vụ độc lập, tách khỏi backend chính, giao tiếp với giao diện qua một điểm cuối riêng. Dịch vụ này áp dụng kiến trúc kết hợp truy hồi và sinh đã trình bày ở mục \ref{subsec:rag}, được chọn nhằm hạn chế hiện tượng bịa đặt thông tin khi trả lời các câu hỏi về quy chế và thông tin học vụ, đúng nhu cầu đã phân tích ở Chương 2.
...
\label{subsec:loadtest}

Khác với số liệu giám sát ở mục trên, kiểm thử tải chủ động tạo tải vượt mức sử dụng thông thường nhằm xác định ngưỡng chịu đựng của hệ thống bằng công cụ chuyên dụng (ví dụ k6), với kịch bản mô phỏng số người dùng đồng thời tăng dần và đo thời gian phản hồi ở các phân vị theo từng mức tải. Tại thời điểm hoàn thành báo cáo, nhóm chưa triển khai được đợt đo này và sẽ bổ sung kết quả trong giai đoạn tiếp theo của đề tài (xem mục \ref{sec:limitations}).

\subsection{Đánh giá hiệu năng theo điểm cuối tiêu biểu}
\label{subsec:apiperf}

Số liệu dưới đây được thu thập từ hệ thống giám sát Prometheus trong điều kiện sử dụng thực tế (không phải kiểm thử tải chủ động), phản ánh thời gian phản hồi trung bình của một số điểm cuối tiêu biểu, mỗi điểm cuối đại diện cho một dạng tải khác nhau.

\renewcommand{\arraystretch}{1.2}
\begin{longtable}{|>{\raggedright\arraybackslash}p{3.6cm}|>{\raggedright\arraybackslash}p{4.2cm}|>{\centering\arraybackslash}p{2.4cm}|>{\raggedright\arraybackslash}p{4.4cm}|}
\caption{Hiệu năng của một số điểm cuối tiêu biểu}\label{tab:apiperf}\\
\hline \textbf{Điểm cuối} & \textbf{Đặc điểm tải} & \textbf{Độ trễ trung bình} & \textbf{Nguyên nhân} \\ \hline
\endfirsthead
\hline \textbf{Điểm cuối} & \textbf{Đặc điểm tải} & \textbf{Độ trễ trung bình} & \textbf{Nguyên nhân} \\ \hline
\endhead
Đăng nhập (\texttt{/api/auth/login}) & Băm mật khẩu bằng BCrypt & $\sim$385,00 ms & Chi phí tính toán hash cao có chủ đích, nhằm tăng khả năng chống dò mật khẩu \\ \hline
Tải danh sách tổ chức (\texttt{/api/organizations}) & Đọc nhiều, dữ liệu ít thay đổi & $\sim$5,79 ms & Phục vụ trực tiếp từ L1 Cache (Caffeine), không truy vấn Database \\ \hline
Lịch sử hội thoại & Đọc lượng dữ liệu lớn, có phân trang & $\sim$44,25 ms & Cursor-based Pagination kết hợp chỉ mục tối ưu, thay cho Offset truyền thống \\ \hline
Danh sách bài viết (Diễn đàn/Tin tức) & Kết nhiều bảng (tác giả, lượt thích, bình luận, quyền truy cập) & $\sim$23,70--26,38 ms & DTO Projection của Spring Data R2DBC, giảm chi phí khởi tạo Entity \\ \hline
\end{longtable}
\renewcommand{\arraystretch}{1}

\subsection{So sánh trước và sau khi áp dụng các biện pháp tối ưu}
\label{subsec:beforeafter}

Việc chuyển từ mô hình xử lý đồng bộ sang kiến trúc Reactive WebFlux kết hợp R2DBC mang lại cải thiện rõ rệt về hiệu năng hệ thống, được ghi nhận qua các chỉ số giám sát Prometheus/Micrometer trước và sau quá trình tối ưu. Đây là phép đo có giá trị chứng minh cao cho luận điểm tái thiết kế của đề tài, vì cho thấy chính các quyết định tối ưu của nhóm tạo ra khác biệt đo được, độc lập với việc so sánh với hệ thống 2024.

\renewcommand{\arraystretch}{1.2}
\begin{longtable}{|>{\raggedright\arraybackslash}p{3.4cm}|>{\raggedright\arraybackslash}p{4.6cm}|>{\raggedright\arraybackslash}p{4.6cm}|}
\caption{So sánh hiệu năng trước và sau khi áp dụng các biện pháp tối ưu}\label{tab:beforeafter}\\
\hline \textbf{Chỉ số} & \textbf{Trước tối ưu} & \textbf{Sau tối ưu} \\ \hline
\endfirsthead
\hline \textbf{Chỉ số} & \textbf{Trước tối ưu} & \textbf{Sau tối ưu} \\ \hline
\endhead
Thời gian phản hồi trung bình & $\sim$450 ms, dễ Timeout khi tải tăng do Thread bị chặn chờ I/O & $\sim$35 ms, ổn định ngay cả khi tải tăng \\ \hline
Thông lượng (RPS) & $\sim$500 request/giây trước khi cạn Thread Pool & $>$4.500 request/giây, duy trì ổn định \\ \hline
Mức sử dụng CPU & 85--90\% do Context Switching giữa hàng nghìn Thread chờ I/O & 35--40\% lúc cao điểm, $\sim$0,7\% lúc nhàn rỗi \\ \hline
Mức sử dụng bộ nhớ Heap & 800 MB--1,2 GB do Hibernate ORM tạo nhiều đối tượng trung gian & $\sim$167 MB nhờ R2DBC kết hợp DTO Projection \\ \hline
Kết nối đồng thời & Từ chối kết nối khi vượt $\sim$2.000, do Thread Pool của Tomcat bị giới hạn & Duy trì ổn định $>$20.000 kết nối WebSocket/SSE, nhờ Event Loop Multiplexing \\ \hline
Connection Pool cơ sở dữ liệu & Dễ cạn kiệt (100/100 Active Connections) khi có truy vấn chậm & Gần như bằng 0 khi không có I/O, luôn ổn định nhờ mô hình Non-blocking \\ \hline
\end{longtable}
\renewcommand{\arraystretch}{1}

Cần nêu rõ một giới hạn về phương pháp. Các chỉ số trên được ghi nhận qua giám sát Prometheus trong quá trình phát triển và thử nghiệm nội bộ, không phải từ một đợt kiểm thử tải chủ động có kiểm soát song song hai phiên bản đồng bộ và bất đồng bộ trên cùng điều kiện hạ tầng, vì hệ thống không duy trì một bản hiện thực đồng bộ song song để đối chứng trực tiếp. Với luận điểm về mô hình phản ứng, báo cáo chủ yếu dựa vào bằng chứng gián tiếp là số kết nối đồng thời duy trì được và số luồng tiêu tốn ở mức tải quan sát, kèm lập luận đối chiếu với đặc điểm của mô hình cấp một luồng cho mỗi yêu cầu.
...
Về phạm vi đánh giá, hệ thống chưa vận hành ở quy mô toàn trường và đợt thử nghiệm với người dùng còn giới hạn như đã phân tích ở mục 5.7. Nhóm cũng chưa thực hiện được đợt kiểm thử tải chủ động bằng công cụ chuyên dụng để xác định ngưỡng chịu đựng của hệ thống, như đã ghi nhận ở mục \ref{subsec:loadtest}; các số liệu hiệu năng ở mục \ref{subsec:apiperf} và \ref{subsec:beforeafter} được thu thập từ giám sát Prometheus trong điều kiện sử dụng thực tế, không thay thế hoàn toàn cho một đợt kiểm thử tải có kiểm soát.
...
\bibitem{skillgap} VOV, "Thiếu định hướng nghề nghiệp, sinh viên dễ chán giảng đường," VOV.vn, 2024. [Online]. Available: \url{https://vov.vn/xa-hoi/thieu-dinh-huong-nghe-nghiep-sinh-vien-de-chan-giang-duong-post1045022.vov}
