...
\begin{tikzpicture}[font=\small,
 act/.style={draw,rounded corners=2pt,fill=blue!10,minimum height=8mm,minimum width=27mm,align=center},
 uc/.style={draw,ellipse,fill=green!8,minimum height=8mm,minimum width=42mm,align=center,inner sep=2pt},
 ln/.style={gray!70}]
\node[act] (g)  at (0,1.7)   {Khách vãng lai};
\node[act] (st) at (0,0.1)   {Sinh viên};
\node[act] (al) at (0,-1.5)  {Cựu sinh viên};
\node[act] (sf) at (0,-3.4)  {Quản trị khoa};
\node[act] (sa) at (0,-5.0)  {Quản trị trường};
\node[uc] (u1) at (7.2,2.2)  {Xem nội dung công khai};
\node[uc] (u2) at (7.2,0.95) {Đăng ký, đăng nhập, xác minh};
\node[uc] (u3) at (7.2,-0.3) {Diễn đàn, tin tức, việc làm};
\node[uc] (u4) at (7.2,-1.55) {Sự kiện và vé điện tử};
\node[uc] (u5) at (7.2,-2.8) {Cố vấn: đăng ký, đặt lịch};
\node[uc] (u6) at (7.2,-4.05) {Nhắn tin và kết nối};
\node[uc] (u7) at (7.2,-5.3) {Đóng góp quỹ};
\node[uc] (u8) at (7.2,-6.55) {Kiểm duyệt và duyệt hồ sơ};
\node[uc] (u9) at (7.2,-7.8) {Quản trị tổ chức và cấu hình};
...
\begin{scope}[on background layer]
\node[draw,dashed,rounded corners,fit={(u1)(u9)},inner sep=4mm] (bx) {};
\end{scope}
\node[font=\small\bfseries,above=1mm of bx] {Hệ thống AlumVerse 2026};
\end{tikzpicture}
...
\begin{figure}[H]\centering
\begin{subfigure}{0.47\textwidth}\centering\includegraphics[width=\linewidth]{web-trang-chu}\caption{Trang chủ}\end{subfigure}\hfill
\begin{subfigure}{0.47\textwidth}\centering\includegraphics[width=\linewidth]{web-co-van}\caption{Danh sách người cố vấn}\end{subfigure}
\vspace{6pt}
\begin{subfigure}{0.47\textwidth}\centering\includegraphics[width=\linewidth]{web-dien-dan}\caption{Diễn đàn nghề nghiệp}\end{subfigure}\hfill
\begin{subfigure}{0.47\textwidth}\centering\includegraphics[width=\linewidth]{web-admin-dashboard}\caption{Bảng điều khiển quản trị}\end{subfigure}
\caption{Một số màn hình tiêu biểu của ứng dụng web}
...
\phfigl{6cm}{Ảnh ghép một số màn hình tiêu biểu của ứng dụng di động, gồm màn hình chọn đơn vị, danh sách sự kiện, hội thoại và màn hình điểm danh.}{Một số màn hình tiêu biểu của ứng dụng di động}{fig:mobilescreens}
...
\begin{tikzpicture}[font=\normalsize,
 hdr/.style={draw,fill=blue!10,rounded corners=2pt,minimum height=8mm,text width=33mm,align=center,inner sep=2pt},
 ll/.style={dashed,gray!55},
 msg/.style={-{Latex[length=2.4mm]},thick},
 ret/.style={-{Latex[length=2.4mm]},thick,dashed},
 lb/.style={above,inner sep=1.6pt,font=\normalsize},
 slb/.style={anchor=east,align=right,font=\normalsize,inner sep=1pt},
 frm/.style={draw=gray!65,thick},
 tab/.style={draw=gray!65,fill=gray!15,text=black,anchor=south west,font=\small\bfseries,inner sep=2.5pt},
 grd/.style={anchor=base west,text=black,font=\small\itshape},
 nt/.style={draw,dotted,rounded corners,align=left,font=\small,inner sep=2.2mm}]
\node[hdr] (h1) at (0.00,0) {Người được\\ cố vấn};
\draw[ll] (h1.south) -- (0.00,-9.99);
\node[hdr] (h2) at (4.55,0) {Giao diện\\ Web / Mobile};
\draw[ll] (h2.south) -- (4.55,-9.99);
\node[hdr] (h3) at (9.10,0) {MenteeController};
\draw[ll] (h3.south) -- (9.10,-9.99);
\node[hdr] (h4) at (13.65,0) {MenteeService};
\draw[ll] (h4.south) -- (13.65,-9.99);
\node[hdr] (h5) at (18.20,0) {PostgreSQL};
\draw[ll] (h5.south) -- (18.20,-9.99);
\draw[msg] (0.00,-1.20) -- node[lb]{Chọn khung giờ, xác nhận đặt lịch} (4.55,-1.20);
\draw[msg] (4.55,-1.94) -- node[lb]{\texttt{POST /mentee/sessions/book}} (9.10,-1.94);
\draw[msg] (9.10,-2.68) -- node[lb]{\texttt{bookSession(request)}} (13.65,-2.68);
\draw[msg] (13.65,-3.42) -- ++(-1.0,0) -- ++(0,-0.44) -- (13.65,-3.86);
\node[slb,text width=41mm] at (12.47,-3.64) {Kiểm tra quyền tham gia};
\draw[msg] (13.65,-4.36) -- node[lb]{\texttt{findById(availabilityId)}} (18.20,-4.36);
\draw[ret] (18.20,-5.10) -- node[lb]{bản ghi khung giờ} (13.65,-5.10);
\draw[msg] (13.65,-5.84) -- ++(-1.0,0) -- ++(0,-0.44) -- (13.65,-6.28);
\node[slb,text width=41mm] at (12.47,-6.06) {Kiểm tra khung giờ hợp lệ};
\draw[msg] (13.65,-6.78) -- node[lb]{\texttt{updateStatus(id, BOOKED)}} (18.20,-6.78);
\draw[msg] (13.65,-7.52) -- node[lb]{\texttt{save(session)}: trạng thái CONFIRMED} (18.20,-7.52);
\draw[ret] (13.65,-8.26) -- node[lb]{\texttt{201 Created} kèm thông tin buổi cố vấn} (4.55,-8.26);
\draw[ret] (4.55,-9.00) -- node[lb]{Hiển thị lịch hẹn đã đặt} (0.00,-9.00);
\node[frm,fit={(14.20,-7.98) (20.05,-3.74)},inner sep=0pt] (fr) {};
\node[tab] at (fr.north west) {giao dịch};
\node[nt,anchor=north west,text width=210mm] at (-1.75,-10.29) {Các bước kiểm tra gồm: tài khoản đã xác minh tổ chức và đã tham gia chương trình cố vấn; khung giờ còn ở trạng thái còn trống, thời điểm bắt đầu chưa trôi qua và người đặt không phải chính người cố vấn. Nếu một bước kiểm tra không đạt, giao dịch bị hủy bỏ: khung giờ giữ nguyên trạng thái còn trống và hệ thống trả về lỗi tương ứng. Thông báo cho người cố vấn được phát bất đồng bộ sau khi giao dịch hoàn tất. Việc kiểm tra trạng thái và thao tác ghi hiện nằm ở tầng ứng dụng bên trong giao dịch; hướng củng cố bằng cập nhật có điều kiện ở tầng cơ sở dữ liệu được nêu ở mục 3.4.3 và Chương 6.};
\end{tikzpicture}
...
\draw[ll] (h1.south) -- (0.00,-12.29);
\node[hdr] (h2) at (4.55,0) {Giao diện\\ Web / Mobile};
\draw[ll] (h2.south) -- (4.55,-12.29);
\node[hdr] (h3) at (9.10,0) {Backend\\ (fundraising)};
\draw[ll] (h3.south) -- (9.10,-12.29);
\node[hdr] (h4) at (13.65,0) {PostgreSQL};
\draw[ll] (h4.south) -- (13.65,-12.29);
\node[hdr] (h5) at (18.20,0) {Cổng thanh toán\\ Sepay};
\draw[ll] (h5.south) -- (18.20,-12.29);
\draw[msg] (0.00,-1.20) -- node[lb]{Chọn quỹ và nhập số tiền đóng góp} (4.55,-1.20);
\draw[msg] (4.55,-1.94) -- node[lb]{\texttt{POST /funds/\{id\}/donations}} (9.10,-1.94);
\draw[msg] (9.10,-2.68) -- node[lb]{Ghi bản ghi đóng góp, trạng thái PENDING} (13.65,-2.68);
\draw[ret] (9.10,-3.42) -- node[lb]{Mã QR kèm nội dung chuyển khoản \texttt{FD<id>}} (4.55,-3.42);
\draw[msg] (0.00,-4.16) -- node[lb]{Quét mã QR và chuyển khoản} (18.20,-4.16);
\draw[msg] (18.20,-4.90) -- node[lb]{webhook kèm \texttt{Apikey} và \texttt{content}} (9.10,-4.90);
\draw[msg] (9.10,-5.64) -- ++(-1.0,0) -- ++(0,-0.44) -- (9.10,-6.08);
\node[slb,text width=41mm] at (7.92,-5.86) {Xác thực \texttt{Apikey}, trích \texttt{FD<id>}};
\draw[msg] (9.10,-6.82) -- node[lb]{\texttt{findById(donationId)}} (13.65,-6.82);
\draw[ret] (13.65,-7.56) -- node[lb]{bản ghi đóng góp} (9.10,-7.56);
\node[grd] at (4.78,-8.46) {[bản ghi đã ở trạng thái SUCCESS]};
\draw[msg] (9.10,-8.94) -- ++(-1.0,0) -- ++(0,-0.44) -- (9.10,-9.38);
\node[slb,text width=32mm] at (7.92,-9.16) {Bỏ qua, không cộng tiền};
\node[grd] at (4.78,-10.08) {[ngược lại]};
\draw[dashed,gray!65] (4.60,-9.68) -- (15.50,-9.68);
\draw[msg] (9.10,-10.56) -- node[lb]{Cập nhật SUCCESS và cộng vào quỹ} (13.65,-10.56);
\draw[ret] (9.10,-11.30) -- node[lb]{\texttt{success = true}} (18.20,-11.30);
\node[frm,fit={(4.60,-11.02) (15.50,-7.68)},inner sep=0pt] (fr) {};
\node[tab] at (fr.north west) {alt};
\node[nt,anchor=north west,text width=210mm] at (-1.75,-12.59) {Hệ thống bỏ qua thông báo có \texttt{transferType} khác \texttt{in}. Cổng thanh toán gửi lại thông báo nhiều lần khi chưa nhận được phản hồi thành công; mọi lần gửi lại đều rơi vào nhánh bỏ qua nên số tiền của quỹ không thay đổi. Trạng thái của chính bản ghi đóng góp đóng vai trò chốt chặn, nhờ đó hệ thống không cần thêm một bộ nhớ đệm riêng để lưu dấu các thông báo đã xử lý. Chi tiết nhánh quyết định được trình bày ở Hình \ref{fig:webhook}.};
\end{tikzpicture}
...
TC-015 & Người cố vấn đã mở một khung giờ rảnh & Hai người được cố vấn lần lượt gửi yêu cầu đặt lịch vào cùng khung giờ đó & Chỉ yêu cầu đầu tiên thành công; yêu cầu sau bị từ chối với thông báo khung giờ không còn trống, khung giờ không bị đặt hai lần \\ \hline
...
Hai test case TC-015 và TC-028 là minh chứng trực tiếp cho hai cơ chế kỹ thuật trọng tâm đã trình bày ở Chương 3 và Chương 4, gồm kiểm soát trùng lịch khi đặt lịch cố vấn và bảo đảm xử lý một lần khi đối soát giao dịch.
