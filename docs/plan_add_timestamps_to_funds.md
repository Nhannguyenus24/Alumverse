...
- **Khởi tạo DB**: Schema được nạp từ `docs/postgre.sql` khi `docker compose` dựng lại container DB → **không cần migration thủ công**, chỉ cần sửa file schema gốc rồi recreate.
...
> Sau khi sửa, chạy lại `docker compose` để dựng lại DB từ schema mới. Không cần file migration riêng.
...
- **Migration thủ công**: bỏ qua — chỉ cần sửa `docs/postgre.sql` và `docker compose` dựng lại DB.
...
1. Sửa `docs/postgre.sql` (mục 2.1).
2. Sửa `Funds.java` (mục 2.2).
3. `docker compose down` (xoá volume DB nếu cần) rồi `docker compose up` để dựng lại DB với schema mới.
