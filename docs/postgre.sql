...
CREATE UNIQUE INDEX ON "peer_verifications" ("organization_id", "target_member_id", "verifier_member_id");
...
-- Một user chỉ xuất hiện một lần trong mỗi tổ chức (thay cho UNIQUE(user_id) đã bỏ)
ALTER TABLE "organization_members" ADD CONSTRAINT "uk_organization_members_org_user" UNIQUE ("organization_id", "user_id");

ALTER TABLE "peer_verifications" ADD FOREIGN KEY ("organization_id") REFERENCES "organizations" ("id");

ALTER TABLE "verification_requests" ADD FOREIGN KEY ("organization_id") REFERENCES "organizations" ("id");

ALTER TABLE "events" ADD FOREIGN KEY ("organization_id") REFERENCES "organizations" ("id");

-- NOTE: Các foreign key từ *_member_id -> organization_members("user_id") đã bị bỏ
-- do UNIQUE(user_id) không còn (một user có thể thuộc nhiều tổ chức). Tính toàn vẹn
-- tham chiếu cho các cột này được đảm bảo ở tầng ứng dụng.
